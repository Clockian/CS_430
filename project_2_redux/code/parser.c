 ...
 *  Improperly designed, as memory is not freed after it is used. While memory is
 *  properly freed in some locations, certain functions are unable to have any freeing
 *  of memory due to their design. All in all, major design flaw is that functions
 *  create and return allocated memory. Fixing this is not feasible at the moment, but
 *  is noted for a future redesign.
 */
 ...
 *  Due to the design of this funtion and file, I am unable to unallocate the
 *  memory created without significant work to rewrite the structure of the file
 */
...
void read_csv_file(FILE *fr, scene_object result[]) {
	...
	while(fgets(str, 99, fr)){
		cleaned_str = remove_brackets_whitespace(str);
